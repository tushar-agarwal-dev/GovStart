...
export default function App() {
  ...
  useEffect(() => {
    if (auth) {
      localStorage.setItem('yuktisetu_auth', JSON.stringify(auth));
      setView('dashboard');
      loadDashboardData();
    } else {
      localStorage.removeItem('yuktisetu_auth');
      setView('landing');
    }
  }, [auth]);
  ...
  const fillCredentials = (role: string) => {
    if (role === 'admin') {
      ...
      setLoginPassword('AdminPass_YuktiSetu_2026!');
    } else if (role === 'dept') {
      setLoginEmail('dept@yuktisetu.gov.in');
      setLoginPassword('DeptPass_YuktiSetu_2026!');
    } else if (role === 'startup') {
      setLoginEmail('ecotech@startups.in');
      setLoginPassword('StartupPass_YuktiSetu_2026!');
    } else if (role === 'expert') {
      setLoginEmail('kulkarni@coep.ac.in');
      setLoginPassword('ExpertPass_YuktiSetu_2026!');
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <DepartmentDashboard 
          ...
          problems={problems}
          pilots={pilots}
          onPostChallenge={() => setView('post-problem')}
          onViewProblem={viewProblemDetails}
          onViewPilot={viewPilotWorkspace}
        ...
        {view === 'login' && (
          <div className="max-w-md mx-auto py-12">
            <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
              ...
              <p className="text-sm text-slate-500 mb-6">Enter your credential details to log in to YuktiSetu.</p>
      ...
      <footer className="bg-slate-900 text-slate-400 py-8 border-t border-slate-800 text-xs">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-200">YuktiSetu Pilot Platform</span>
            <span>&bull;</span>
